from flask import Blueprint, g, jsonify, render_template, request

from .userController import login_required, role_required
from ..Models import Message
from ..database import db

message_bp = Blueprint("messages", __name__, url_prefix="/messages")


def _payload():
    return request.get_json(silent=True) or request.form.to_dict(flat=True)


def _message_dict(message):
    sender_name = message.sender.name if message.sender else message.senderID
    receiver_name = message.receiver.name if message.receiver else message.receiverID
    return {
        "messageID": message.messageID,
        "senderID": message.senderID,
        "senderName": sender_name,
        "senderRole": message.sender.role if message.sender else "alumni",
        "receiverID": message.receiverID,
        "receiverName": receiver_name,
        "receiverRole": message.receiver.role if message.receiver else "alumni",
        "content": message.content,
        "timestamp": message.timestamp.isoformat(),
        "status": message.status,
        "attachments": message.attachments or [],
    }


@message_bp.get("")
@login_required
def inbox_page():
    inbox = (
        Message.query.filter_by(receiverID=g.current_user.userID)
        .order_by(Message.timestamp.desc())
        .limit(50)
        .all()
    )
    return render_template("messages.html", inbox=inbox)


@message_bp.get("/inbox")
@login_required
def inboxAPI():
    """API endpoint for frontend to fetch messages"""
    inbox = (
        Message.query.filter_by(receiverID=g.current_user.userID)
        .order_by(Message.timestamp.desc())
        .limit(50)
        .all()
    )
    return jsonify({"messages": [_message_dict(m) for m in inbox]})


@message_bp.get("/sent")
@login_required
def sentAPI():
    """API endpoint to get sent messages"""
    sent = (
        Message.query.filter_by(senderID=g.current_user.userID)
        .order_by(Message.timestamp.desc())
        .limit(50)
        .all()
    )
    return jsonify({"messages": [_message_dict(m) for m in sent]})


@message_bp.post("/request")
@role_required("alumni", "admin")
def requestMessage():
    data = _payload()
    receiver_id = (data.get("receiverID") or "").strip()
    content = (data.get("content") or "I'd like to message you.").strip()
    if not receiver_id:
        return jsonify({"error": "receiverID is required"}), 400

    message = Message(
        senderID=g.current_user.userID,
        receiverID=receiver_id,
        content=content,
        status="requested",
        attachments=data.get("attachments") or [],
    )
    db.session.add(message)
    db.session.commit()
    return jsonify({"message": "Message request sent", "messageID": message.messageID}), 201


@message_bp.post("/<message_id>/accept")
@role_required("alumni", "admin")
def acceptRequest(message_id):
    message = db.session.get(Message, message_id)
    if not message:
        return jsonify({"error": "Message not found"}), 404
    if message.receiverID != g.current_user.userID:
        return jsonify({"error": "Only receiver can accept this request"}), 403

    message.status = "accepted"
    db.session.commit()
    return jsonify({"message": "Message request accepted", "messageID": message.messageID})


@message_bp.post("/<message_id>/reject")
@role_required("alumni", "admin")
def rejectMessage(message_id):
    message = db.session.get(Message, message_id)
    if not message:
        return jsonify({"error": "Message not found"}), 404
    if message.receiverID != g.current_user.userID:
        return jsonify({"error": "Only receiver can reject this request"}), 403

    message.status = "rejected"
    db.session.commit()
    return jsonify({"message": "Message request rejected", "messageID": message.messageID})


@message_bp.post("")
@role_required("alumni", "admin")
def sendMessage():
    data = _payload()
    receiver_id = (data.get("receiverID") or "").strip()
    content = (data.get("content") or "").strip()
    if not receiver_id or not content:
        return jsonify({"error": "receiverID and content are required"}), 400

    message = Message(
        senderID=g.current_user.userID,
        receiverID=receiver_id,
        content=content,
        status="sent",
        attachments=data.get("attachments") or [],
    )
    db.session.add(message)
    db.session.commit()
    return jsonify({"message": "Message sent", "data": _message_dict(message)}), 201