...
from App.Controllers.auth import decode_access_token, extract_bearer_token, issue_access_token
from ..Models import Admin, Alumni, Profile, User
...
def get_current_user():
    ...
    if token:
        payload, _error = _decode_access_token(token)
        if payload:
    ...
    # Legacy fallback while frontend migrates to JWT headers.
...
def login_page():
    if _wants_json():
        return jsonify({"message": "Use POST /api/users/login"}), 200
...
def register_page():
    if _wants_json():
        return jsonify({"message": "Use POST /api/users/register"}), 200
